...
<table width="1200" align="center" class="mt-3 tbd">
      ...
      <td width="1000" valign="top" class="pr-4"> <span style="font-weight: 700">Chờ thanh toán</span>
      	<table class="table table-hover mt-2 table-bordered">
 			...
 			<%
 				ArrayList<hoadonbean> lstHoaDon = (ArrayList<hoadonbean>)request.getAttribute("listhoadon");
 				for(hoadonbean s: lstHoaDon){
 			%>
      		
      		<tbody>
      			<tr>
      			<% if(s.getDamua()==0) {%>
      			<form action="lichsumuahangservlet" method="post" class="input-group pl-4 pr-4"> 	<!-- Bắt đầu form sửa xóa -->   
          			<td> <%=s.getMaHoaDon()%></td>
           			<td> <%=s.getMakh()%> </td>
           			<td> <%=s.getNgayMua()%> </td>
           			<td><%=s.getDamua()==0?"Chưa mua":"Đã mua"%></td>
           			<td><%=request.getAttribute("tenmua")%></td>
      			</tr>
      			<tr>
      				<td  colspan="6">
      					<div class="butmuahang d-flex justify-content-center">
      						<button type="submit" name="butxacnhanmua" value="<%=s.getMaHoaDon() %>" class="btn btn-success btn-check-out pl-4 pr-4">
